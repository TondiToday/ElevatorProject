...
#include "Utilities.h"
	...
	void dispatch(vector<Elevator*>& d_elevators, int& direction, int& floor_request)
	{
		...
		{
			// adds floor request for floor request tracking
			d_elevators[index_of_selected_elevator]->floorRequests_down.push_down(floor_request);
		...
		{
			// adds floor request for floor request tracking
			d_elevators[index_of_selected_elevator]->floorRequests_up.push_up(floor_request);
