...
struct node
{
	vector<int> data;// data stored in the node
	node * next; // pointer to point to the next node
	node(vector<int> x, node *y) { data = x; next = y; }// default constructior
};
class q_modified
{
protected:
...
public:
	...
	void push_up(vector<int> x);// to store requestes in increasing order (1,2,3,4,5)
	void push_down(vector<int> x);// to store requestes in decreasing order (5,4,,3,2,1)
	int print();// print out the stored data in the increasing queue 
	vector<int> front_value();// to return the front value of the queue
	vector<int> back_value(); // to return the back value of the queue
